...
BUILD_ID='SP-RENDER-GOOGLECOORDS-2026-08-12'
...
def _google_geocode(addr):
    ...
    r=HTTP.get('https://maps.googleapis.com/maps/api/geocode/json?'+params,timeout=10)
    try:d=r.json()
    except Exception:raise ValueError('Resposta inválida da Google Geocoding API')
    if not r.ok:
        raise ValueError(d.get('error_message') or f'Google Geocoding HTTP {r.status_code}')
    status=d.get('status')
    if status=='ZERO_RESULTS':return None
    if status!='OK':raise ValueError(d.get('error_message') or f'Google Geocoding: {status}')
    result=(d.get('results') or [None])[0]
    if not result:return None
    geo=result.get('geometry') or {};loc=geo.get('location') or {}
    lat=num(loc.get('lat'));lng=num(loc.get('lng'))
...
def google_geocode_existing_pevs(token):
    if not GOOGLE_MAPS_API_KEY:
        raise ValueError('GOOGLE_MAPS_API_KEY não está configurada no Render.')
    pevs=Supa.get('pevs',token,{'deleted_at':'is.null','active':'eq.true','select':'id,name,cep,street,number,district,city,state','order':'id.asc'})
    updated=[];failed=[]
    for pev in pevs:
        try:
            g=_google_geocode(pev)
            if not g:
                failed.append({'id':pev['id'],'name':pev['name'],'error':'Endereço não encontrado pelo Google'})
                continue
            confirmed=(g.get('location_type')=='ROOFTOP')
            Supa.update('pevs',token,{'id':f'eq.{pev["id"]}'},{
                'lat':g['lat'],'lng':g['lng'],'location_confirmed':confirmed
            })
            updated.append({
                'id':pev['id'],'name':pev['name'],'lat':g['lat'],'lng':g['lng'],
                'location_type':g.get('location_type'),'formatted_address':g.get('formatted_address'),
                'place_id':g.get('place_id'),'confirmed':confirmed
            })
        except Exception as e:
            failed.append({'id':pev['id'],'name':pev['name'],'error':str(e)})
    return {'total':len(pevs),'updated':updated,'failed':failed}
...
def geocode_address(addr):
    ...
    if GOOGLE_MAPS_API_KEY:
        try:
            g=_google_geocode(addr)
            if g: result=(g['lat'],g['lng'])
        except Exception as e:
            print('[GOOGLE GEOCODE]',e)
    if result is None:
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def api_get(self,path):
        try:
            if path=='/api/health':return self.send_json({'ok':True,'build':BUILD_ID,'listen':f'{HOST}:{PORT}','render':IS_RENDER,'external_url':os.environ.get('RENDER_EXTERNAL_URL',''),'google_geocoding':bool(GOOGLE_MAPS_API_KEY)})
    ...
    def api_write(self,method,path):
        ...
        try:
            ...
            if path=='/api/pevs/geocode-google' and method=='POST':
                if role!='admin':return self.send_json({'error':'Sem permissão'},403)
                print(f'[GOOGLE GEOCODE] build={BUILD_ID} atualizando PEVs existentes')
                result=google_geocode_existing_pevs(t)
            ...
            if path=='/api/pevs' and method=='POST':
                ...
                return self.send_json({'id':row.get('id') if isinstance(row,dict) else None},201)
