...
async function renderPevs() {
  ...
  $('#page').innerHTML = `
    <div class="page-head"><div><span class="eyebrow">Cadastros</span><h1>PEVs / Locais</h1><p class="muted">Endereços permanentes, responsáveis, horários e coordenadas.</p></div>${canEdit?`<div class="page-head-actions"><button id="addPev" class="btn primary">+ Novo PEV</button>${state.user.role==='admin'?'<button id="googleCoords" class="btn secondary">Atualizar coordenadas Google</button><button id="pevTrash" class="btn secondary">Lixeira</button>':''}</div>`:''}</div>
  ...
  if ($('#googleCoords')) $('#googleCoords').onclick = async () => {
    if(!confirm('Recalcular latitude e longitude de TODAS as PEVs ativas usando o Google?\n\nAs coordenadas atuais serão substituídas.')) return;
    const btn=$('#googleCoords'); const oldText=btn.textContent; btn.disabled=true; btn.textContent='Consultando Google...';
    try{
      const r=await api('/api/pevs/geocode-google',{method:'POST',body:{}});
      const ok=(r.updated||[]).length, fail=(r.failed||[]).length;
      toast(`${ok} PEV(s) atualizadas pelo Google${fail?` • ${fail} não localizada(s)`:''}.`, fail?'error':'success');
      if(fail) console.warn('PEVs não localizadas pelo Google:',r.failed);
      await renderPevs();
    }catch(e){toast(e.message,'error')}finally{if(btn){btn.disabled=false;btn.textContent=oldText}}
  };
...
function openPevModal(pev=null) {
  ...
  form.onsubmit = async e => {
    ...
    try { await api(pev?`/api/pevs/${pev.id}`:'/api/pevs',{method:pev?'PUT':'POST',body}); $('#modal').close(); toast('PEV salvo.','success'); renderPevs(); } catch(err){toast(err.message,'error');}
  };
}
