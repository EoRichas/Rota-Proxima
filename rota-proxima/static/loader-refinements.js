(() => {
  const TRUCK_MARK = 'cassola-loader-v2';

  function patchTruck(root = document) {
    root.querySelectorAll?.('.rp-page-loader .rp-truck-body svg').forEach(svg => {
      if (svg.dataset.loaderVersion === TRUCK_MARK) return;
      svg.dataset.loaderVersion = TRUCK_MARK;
      svg.setAttribute('viewBox', '0 0 198 93');
      svg.innerHTML = `
        ...
        <rect stroke-width="3" stroke="#282828" fill="#F2C744" rx="2.5" height="90" width="121" y="1.5" x="6.5" />
        <rect stroke-width="2" stroke="#282828" fill="#F2C744" rx="2" height="4" width="6" y="84" x="1" />
        <text x="67" y="48" text-anchor="middle" dominant-baseline="middle" class="cassola-loader-wordmark">CASSOLA</text>
        <path d="M25 61h84" stroke="#145d43" stroke-width="2" stroke-linecap="round" opacity=".6" />
      `;
    });

    root.querySelectorAll?.('.rp-page-loader .rp-lamp-post').forEach(svg => {
      if (svg.dataset.loaderVersion === TRUCK_MARK) return;
      svg.dataset.loaderVersion = TRUCK_MARK;
      svg.setAttribute('viewBox', '0 0 453.459 453.459');
    ...
    const observer = new MutationObserver(records => {
      for (const record of records) {
        for (const node of record.addedNodes) {
          ...
          if (node.matches?.('.rp-page-loader') || node.querySelector?.('.rp-page-loader')) patchTruck(node);
        }
      }
    });
